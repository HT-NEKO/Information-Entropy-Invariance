...
class BertUnpadSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.kyw_norm_method=config.kyw_norm_method # "noCosScale"
        self.kyw_norm_scale=config.kyw_norm_scale   # 32
        self.kyw_normalization=config.kyw_normalization # "softmax"
    ...
    def forward(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor,
        ...
        if self.p_dropout or flash_attn_qkvpacked_func is None:
            ...
            if self.kyw_norm_method=="CosScale":
                ...
                q = q / (q_norm + 1e-9)
                k = k / (k_norm + 1e-9)
                q*=self.kyw_norm_scale
                q*=self.attention_head_size ** 0.5
            #-cosscale-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
            ...
            if self.kyw_normalization == "InfoScale":
                # print("InfoScale")
                l = attn_mask.sum(-1, keepdim=True)
                train_len_ori = 512
                # train_len = q.shape[2]
                head_dim = self.num_attention_heads*self.attention_head_size
                InfoScale_clip = (1-(train_len_ori**(-2/(head_dim))))**0.5 
                InfoScale = (1-(l**(-2/(head_dim))))**0.5
                InfoScale_tensor = torch.tensor(InfoScale, device=attention_scores.device, dtype=attention_scores.dtype) / torch.tensor(InfoScale_clip, device=attention_scores.device, dtype=attention_scores.dtype)
                InfoScale_tensor=InfoScale_tensor.unsqueeze(-1).unsqueeze(-1)
                attention_scores = attention_scores * InfoScale_tensor
                attention_probs=nn.functional.softmax(attention_scores, dim=-1)
            else:
                attention_probs=nn.functional.softmax(attention_scores, dim=-1)
            #-infoscale-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
