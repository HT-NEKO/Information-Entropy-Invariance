...
from .layer import GAULayer, Norm
import ipdb
...
class GAUAlphaEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [
                GAULayer(
                    ...
                    my_gau_layer_num=i,
                    my_info_dict=config.my_info_dict,
                )
        ...
        if config.my_info_dict.get("algorithm",False)=="YaRN":
            sinusoidal_id = self.get_sinusoidal_id_yarn(
                config.max_position_embeddings, config.attention_key_size
            )
            self.register_buffer("sin_pos", sinusoidal_id.sin() * self.mscale, persistent=False)
            self.register_buffer("cos_pos", sinusoidal_id.cos() * self.mscale, persistent=False)
        elif config.my_info_dict.get("algorithm",False)=="PI":
            sinusoidal_id = self.get_sinusoidal_id_PI(
                config.max_position_embeddings, 
                config.attention_key_size,
                4 if config.my_info_dict["task"]=="train" else config.max_position_embeddings/self.config.my_info_dict["train_len"]
            )
    ...
    def get_sinusoidal_id_PI(self,max_length, output_dim, pi_factor):
        position_ids = torch.arange(0, max_length, dtype=torch.float32)
        indices = torch.arange(0, output_dim // 2, dtype=torch.float32)
        indices = torch.pow(10000.0, -2 * indices / output_dim)
        sinusoidal_id = torch.einsum("n,d->nd", position_ids, indices)
        sinusoidal_id = sinusoidal_id * pi_factor
        return sinusoidal_id[None, :, :]

    def get_sinusoidal_id_yarn(self, max_length, output_dim):
        if self.config.my_info_dict["task"]=="train":
            scale=self.config.my_info_dict["scale"]
        else:
            scale=self.config.my_info_dict["seq_len"]/self.config.my_info_dict["train_len"]

        pos_freqs = 10000.0 ** (torch.arange(0, output_dim, 2).float() / output_dim)
        inv_freq_extrapolation = 1.0 / pos_freqs 
        inv_freq_interpolation = 1.0 / (scale * pos_freqs)

        low, high = _yarn_find_correction_range(32, 1, output_dim, 10000.0, self.config.my_info_dict["train_len"])
        inv_freq_mask = (1 - _yarn_linear_ramp_mask(low, high, output_dim // 2).float()) * 1
        
        inv_freq = inv_freq_interpolation * (1 - inv_freq_mask) + inv_freq_extrapolation * inv_freq_mask

        self.register_buffer("inv_freq", inv_freq, persistent=False)
        self.mscale = float(_yarn_get_mscale(scale) * 1)
